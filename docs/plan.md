...
> **⚠️ Отклонение реализации P0 (факт на 27.07.2026).** Собранный прототип отходит от таблицы выше: часть инфраструктуры упрощена. Таблица описывает целевую архитектуру, к которой возвращаемся при росте функциональности (P1+); ниже — что фактически в коде P0.
>
> | Заложено выше | Фактически в P0 (`MainActivity.kt`) | Статус |
> |---|---|---|
> | Room (поверх SQLite) | `SharedPreferences` + JSON-блок (`LocalMoodStore`) | ⚠️ смена БД — **на подтверждении команды** (конституция: спросить перед сменой стека) |
> | Hilt | без DI, весь код в одном файле (без слоёв ViewModel/Repository) | упрощение |
> | Navigation Compose | ручной `when(screen)` state machine | упрощение |
> | Vico | Custom `Canvas` (`MiniChart`) | по плану — санкционированный fallback из таблицы рисков плана |
> | Coroutines + Flow (реактивность) | синхронный пересчёт в памяти при `refresh()` | упрощение; пересчёт после правки/удаления работает |
>
> Причины: минимизация зависимостей для демо-прототипа и обход риска пробелов в Vico. «Локальное хранение на устройстве» и privacy-first соблюдены. **Открытый пункт:** зафиксировать смену БД Room→`SharedPreferences` с командой (Никита — код, Ярослав — спека) и при необходимости обновить конституцию.
